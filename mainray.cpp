...
void LoadPieceTextures() {
    std::string basePath = getBasePath(); // Release Path
...
int main() {
    ...
    bool promotionClickReady = false;
    bool mainMenu = true;
    Colors winner = N;
    Handler H;
    ...
    while (!WindowShouldClose()) {
        ...
        if (!mainMenu && !promotionMenu && !win && !tie){
                        ...
                        if (pieceColor == H.getcurrentcolor()) {
                            ...
                            if (selectedX != -1 && selectedY!=-1){
                                ...
                                if (H.makemove(fromX, fromY, toX, toY,false)){
                                    ...
                                    // snprintf(text, sizeof(text), "%d", moveCount); Debug Move Counter
                                    if(H.getPromotion()){
                                            promotionMenu = true;
                                            xprom = gridX;
                                            yprom = gridY;
            ...
            // DrawText(text, GetScreenWidth() - 30, 10, 44, YELLOW); Debug MoveCounter
        }else{
            if(win) {
                ...
                DrawText(
                    ...
                    if (IsKeyPressed(KEY_R)) {
                        ...
                        prevMovX = -1, prevMovY = -1, prevMovX2 = -1, prevMovY2 = -1;
                        incheckXY = -1, incheckX = -1, incheckY = -1;
                        xprom = -1, yprom = -1;
            ...
            if(promotionMenu){
                float drawX =  xprom * cellSize;
                float drawY = (yprom == 0) ?  yprom * cellSize : (yprom - 3) * cellSize ;
                const char* piecess = (yprom == 0) ? "QNRB" : "qnrb";
                DrawRectangle(drawX, drawY, cellSize, cellSize * 4, WHITE);
                DrawRectangleLines(drawX, drawY, cellSize, cellSize * 4, BLACK);
                for (int i = 0; i < 4; i++) {
                    auto it = pieceTextures.find(piecess[i]);
                    Texture2D &tex = it->second;
                    float scale = (float)cellSize / tex.height;
                    DrawTextureEx(tex, (Vector2){drawX, drawY}, 0.0f, 1, WHITE);
                    drawY += cellSize;
                }

                if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
                    promotionClickReady = true;
                }
                if (promotionClickReady && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) { //wait a second
                    char piece = '-';
                    if (gridX == xprom) {
                        if (H.getcurrentcolor()==W){
                            switch (gridY)
                            {
                            case 7:
                                piece = 'b';
                        ...
                        if (piece != '-' && H.promotion(gridX,7-yprom,piece)){
                            promotionMenu = false;
                            promotionClickReady = false;
                            check = H.incheck(H.getcurrentcolor(),true);
                            if (check){
                                win = H.checkMate();
                                char offset=(H.getcurrentcolor() == W) ? 0 : 32;
                ...
                if (gridX == xprom && H.getcurrentcolor()==W && gridY>3)
                    DrawRectangleLines(gridX * cellSize, gridY * cellSize, cellSize, cellSize, RED);
                else{
                    if (gridX == xprom && H.getcurrentcolor()==B && gridY<4)
                        DrawRectangleLines(gridX * cellSize, gridY * cellSize, cellSize, cellSize, RED);
                }
            }
            if(mainMenu){
                // --- BACKGROUND: faint chessboard ---
                int cell = 60; // 480 / 8 = 60
                for (int y = 0; y < 8; y++) {
                    for (int x = 0; x < 8; x++) {
                        bool dark = (x + y) % 2;
                        Color c = dark ? Fade(BLACK, 0.2f) : Fade(WHITE, 0.2f);
                        DrawRectangle(x * cell, y * cell, cell, cell, c);
                    }
                }

                // Optional slight dark overlay to improve readability
                DrawRectangle(0, 0, 480, 480, Fade(BLACK, 0.35f));


                // --- TITLE ---
                const char* title = "Chess Classy";
                int fontSize = 50;
                int titleWidth = MeasureText(title, fontSize);

                DrawText(
                    title,
                    240 - titleWidth/2,     // centered
                    60,                     // good spacing in 480 window
                    fontSize,
                    WHITE
                );
                ...
                float x = 240 - btnW/2;
                float yStart = 160;   // plenty of room
                ...
                MenuButton buttons[3] = {
                    { "Local Game",       yStart },
                    { "Host Online Game", yStart + 70 },
                    { "Join Online Game", yStart + 140 }
                };
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    Rectangle r = { x, buttons[i].y, btnW, btnH };
                    bool hovered = CheckCollisionPointRec(mouse, r);

                    Color bg = hovered ? Fade(WHITE, 0.4f) : Fade(WHITE, 0.2f);
                    DrawRectangleRec(r, bg);
                    DrawRectangleLinesEx(r, 2, WHITE);

                    int tw = MeasureText(buttons[i].label, 24);
                    DrawText(buttons[i].label,
                            x + btnW/2 - tw/2,
                            buttons[i].y + btnH/2 - 12,
                            24,
                            WHITE);
                ...
                // --- HANDLE CLICKS ---
                if (clicked == 0) {
                    mainMenu = false;  // start local game
                }
                if (clicked == 1) {
                    // Placeholder for host menu
                }
                if (clicked == 2) {
                    // Placeholder for join menu
                }
            }
        }
        EndDrawing();
